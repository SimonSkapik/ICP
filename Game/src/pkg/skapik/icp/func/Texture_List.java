package pkg.skapik.icp.func;

public class Texture_List {
	
	public static final int GRASS = 0;
	public static final int STONE = 1;
	public static final int DIRT = 2;
	public static final int GRASS_SIDE = 3;
	public static final int PLANKS = 4;
	public static final int STONE_BRICK = 5;
	public static final int STONE_SLAB = 6;
	public static final int BRICKS = 7;
	public static final int TNT_SIDE = 8;
	public static final int TNT_TOP = 9;
	public static final int TNT_BOT = 10;
	public static final int COBWEB = 11;
	public static final int COBBLESTONE = 16;
	public static final int MOSSY_COBBLESTONE = 36;
	public static final int BEDROCK = 17;
	public static final int SAND = 18;
	public static final int GRAVEL = 19;
	public static final int OAK_LOG_SIDE = 20;
	public static final int OAK_LOG_CAP = 21;
	public static final int OAK_LEAFS = 53;
	public static final int PALM_LOG_SIDE = 213;
	public static final int PALM_LOG_CAP = 214;
	public static final int PALM_LEAFS = 215;
	public static final int SPRUCE_LOG_SIDE = 116;
	public static final int SPRUCE_LOG_CAP = 132;
	public static final int SPRUCE_LEAFS = 133;
	public static final int ORE_COAL = 34;
	public static final int ORE_IRON = 33;
	public static final int ORE_GOLD = 32;
	public static final int ORE_LAPIS = 160;
	public static final int ORE_REDSTONE = 51;
	public static final int ORE_DIAMOND = 50;
	public static final int WATER = 206;
	public static final int WORKBENCH_SIDE = 60;
	public static final int WORKBENCH_TOP = 43;
	public static final int WORKBENCH_BOT = 74;
	
	
	public static final int SWORD = 199;
	public static final int PICKAXE = 200;
	public static final int AXE = 201;
	public static final int SHOVEL = 202;
	/*public static final int TNT_BOT = 10;
	public static final int TNT_BOT = 10;
	public static final int TNT_BOT = 10;
	public static final int TNT_BOT = 10;
	public static final int TNT_BOT = 10;
	public static final int TNT_BOT = 10;
	public static final int TNT_BOT = 10;
	public static final int TNT_BOT = 10;
	public static final int TNT_BOT = 10;
	public static final int TNT_BOT = 10;
	public static final int TNT_BOT = 10;
	
	*/
	
	
	public static final int UNKNOWN = 198;
	public static final int HBAR_BG = 170;
	public static final int HBAR_BG_SEL = 171;
	public static final int HBAR_BG_HOLD = 168;
	public static final int CROSS = 172;
	public static final int HP_BAR_0 = 188;
	public static final int HP_BAR_1 = 189;
	public static final int HP_BAR_2 = 173;
	public static final int GRASS_PATCH = 56;
	public static final int ROSE = 12;
	public static final int DANDELION = 13;
	
	
}

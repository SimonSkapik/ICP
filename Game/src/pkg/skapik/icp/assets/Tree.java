...
public class Tree {
	...
	private void growPalm(Block[][][] chunk_map, PrintWriter out) {
		...
		// top leaves
		chunk_map[x][y][z] = new Block(Block.OAK_LEAF);
		out.println(x+";"+y+";"+(z)+";"+Block.OAK_LEAF);
		...
		for(int i = x-2; i <= x+2; i+=4){
			chunk_map[i][y-1][z-2] = new Block(Block.OAK_LEAF );
			out.println(i+";"+(y-1)+";"+(z-2)+";"+Block.OAK_LEAF);
			chunk_map[i][y-1][z+2] = new Block(Block.OAK_LEAF);
			out.println(i+";"+(y-1)+";"+(z+2)+";"+Block.OAK_LEAF);
		}
		// direction of trunk
		...
		// trunk
		chunk_map[x][y-1][z] = new Block(Block.OAK_LOG);
		out.println(x+";"+(y-1)+";"+z+";"+Block.OAK_LOG);
		if(trunk == 1){		
			chunk_map[x1][y-2][z1] = new Block(Block.OAK_LOG);
			out.println((x1)+";"+(y-2)+";"+(z1)+";"+Block.OAK_LOG);
			for(int i = y-3; i >= y-height; i--){
				chunk_map[x2][i][z2] = new Block(Block.OAK_LOG);
				out.println((x2)+";"+i+";"+(z2)+";"+Block.OAK_LOG);
			}
		}else{
			for(int i = y-2; i >= y-4; i--){
				chunk_map[x1][i][z1] = new Block(Block.OAK_LOG);
				out.println((x1)+";"+(i)+";"+(z1)+";"+Block.OAK_LOG);
			}
			for(int i = y-5; i >= y-height; i--){
				chunk_map[x][i][z] = new Block(Block.OAK_LOG);
				out.println((x)+";"+i+";"+(z)+";"+Block.OAK_LOG);
			}
		}
	}
